...
function cmdEcho(args) {
    // Join all arguments to form the output string
    const output = args.join(" ");
    // Display the output in the terminal
    addPreformattedText(output, defaultColor);
}
...
function cmdAbout(args) {
    ...
    This terminal based portfolio is inspired by <a class="highlight" href="https://ravitheja.dev">Ravi</a>\n
...
function cmdExit(args) {
    addPreformattedText("Attempting to close tab...", infoColor);

    setTimeout(() => {
        // Check if the tab was opened by the script or if it is the main tab
        if (window.confirm("Do you really want to close this tab?")) {
            window.close();
            // If the window is not closed by the script, notify the user
            if (!window.closed) {
                addPreformattedText("Please close this tab manually.", errorColor);
            }
        } else {
            addPreformattedText("Exit command cancelled.", infoColor);
        }
    }, 1000);
}
